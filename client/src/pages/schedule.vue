<template>
  <v-container>
      ...
      <v-col cols="12">
        <div class="text-left">
          <blockquote class="body-2 font-weight-medium">
            <code>Joshua 1:8 (NIV)</code> &nbsp;
            <br />Keep this Book of the Law always on your lips;
    ...
    <v-row>
      <v-col cols="7" offset="1">
        <v-slider
          ...
          ticks="always"
          tick-size="4"
          hint="Use the Slider to Chose Meditation Interval"
          persistent-hint
        ></v-slider>
      </v-col>
      <v-col cols="3">
        <span class="overline">
          Shuffle every
          <span class="font-weight-bold">{{ timeDisplay }}</span>
        </span>
      </v-col>
    </v-row>
    <v-row>
    ...
    <v-row class="mb-3">
      <v-col cols="10" offset="1">
        <v-textarea autofocus clearable label="Meditation Note" outlined rows="1" v-model="note"></v-textarea>
      </v-col>
    </v-row>
    <v-row>
      <v-col cols="5" offset="6">
        <div class="text-right float-right">
          <v-btn text color="primary" @click="addNote">
            <v-icon left>mdi-plus</v-icon>Add To Meditations
          </v-btn>
        </div>
      </v-col>
    </v-row>
    <v-row v-if="notes.length > 0">
      ...
      <v-col cols="2" offset="10" class="mb-5">
        <v-btn :loading="publishing" color="primary" @click="publish">
          <v-icon left>mdi-cloud</v-icon>Publish
        </v-btn>
      </v-col>
    </v-row>
    ...
    <v-snackbar v-model="snackbar" color="error">
      {{errMsg}}
      <v-btn @click="snackbar = false" text dark>OK</v-btn>
    </v-snackbar>
  </v-container>
</template>
...
export default {
  ...
  computed: {
    timeDisplay() {
      if (this.time < 59) {
        return `${this.time} Mins`;
      }
      const hr = parseInt(this.time / 60);
      const mins = this.time % 60;
      let str = `${hr} Hr${hr === 1 ? "" : "s"}`;
      if (mins) {
        str = `${str} ${mins} Mins`;
      }
      return str;
  ...
  methods: {
    ...
    trunc(w, characters = 60) {
      ...
      return w.length < characters ? w : w.substr(0, characters - 3) + "...";
    }
  }
};
</script>
