<template>
    <div>
            ...
            <v-sheet class="text-center rounded" dark height="200px" inset>
                ...
                <v-container>
                    ...
                    <v-row>
                        ...
                        <v-col cols="4" xs="12">
                            <v-chip pill  v-ripple="{ class: 'pink--text'}" :disabled="notifications.length < 1" class="no-select">
                                Notifications
                        ...
                        <v-col cols="4" xs="12">
                            <div class="text-right">
                                <small class="subtitle-2 font-weight-thin grey--text no-select">Made with <span class="red--text">&hearts;</span>
                                    from EaseCore
                                </small>
                            </div>
                        </v-col>
                ...
                <v-bottom-navigation :value="currentRoute" absolute color="purple lighten-2" grow horizontal
                                     style="box-shadow: none;">
                    <v-btn @click="routeTo('scribe')" value="scribe">
                        <span class="font-weight-bold ">Scribe</span>
                        <v-icon color="purple--text lighten-2">mdi-voice</v-icon>
                    </v-btn>

                    <v-btn @click="routeTo('schedule')" value="schedule">
                        <span class="font-weight-bold">Scheduler</span>
                        <v-icon>mdi-timetable</v-icon>
                    </v-btn>

                    <v-btn @click="routeTo('subtitle')" value="subtitle">
                        <span class="font-weight-bold">TV Subtitles</span>
                        <v-icon>mdi-subtitles</v-icon>
                    </v-btn>

                    <v-btn @click="routeTo('settings')" value="settings">
        ...
        <div @click="showMenu" class="lift-footer z-index-100" id="footer"
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.currentRoute = this.$router.currentRoute && this.$router.currentRoute.name && this.$router.currentRoute.name.toLowerCase();
