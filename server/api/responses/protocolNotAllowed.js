/**
 * protocolNotAllowed.js
 *
 * A custom response.
 *
 * Example usage:
 * ```
 *     return res.protocolNotAllowed();
 *     // -or-
 *     return res.protocolNotAllowed(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function protocolNotAllowed(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info("Ran custom response: res.protocolNotAllowed()");
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info(
      "Custom response `res.protocolNotAllowed()` called with an Error:",
      optionalData
    );
